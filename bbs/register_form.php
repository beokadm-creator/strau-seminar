...
if ($w == "") {
    ...
    if (!isset($_POST['agree']) || !$_POST['agree']) {
        $ref = '';
        if (isset($_REQUEST['referral_code']) && $_REQUEST['referral_code']) { $ref = trim($_REQUEST['referral_code']); }
        else if (function_exists('get_session')) { $s = get_session('referral_code'); if ($s) $ref = $s; }
        if ($ref !== '') { if (function_exists('set_session')) set_session('referral_code', $ref); }
        $redir = G5_BBS_URL.'/register.php' . ($ref !== '' ? ('?referral_code='.urlencode($ref)) : '');
        alert('회원가입약관의 내용에 동의하셔야 회원가입 하실 수 있습니다.', $redir);
    }
    ...
    if (!isset($_POST['agree2']) || !$_POST['agree2']) {
        ...
        if ($ref !== '') { if (function_exists('set_session')) set_session('referral_code', $ref); }
        $redir = G5_BBS_URL.'/register.php' . ($ref !== '' ? ('?referral_code='.urlencode($ref)) : '');
        alert('개인정보 수집 및 이용의 내용에 동의하셔야 회원가입 하실 수 있습니다.', $redir);
    }
