...
if($w == 'a' && $write['qa_email_recv'] && trim($write['qa_email'])) {
    ...
    mailer($config['cf_admin_email_name'], COMMON_SEND_EMAIL, $write['qa_email'], $subject, $content, 1);
}
...
if(($w == '' || $w == 'r') && trim($qaconfig['qa_admin_email'])) {
    ...
    $content = nl2br(conv_unescape_nl(stripslashes($qa_content)));
    $content .= "<br> 이름 : {$qa_name} <br> 이메일 : {$qa_email} <br> 연락처 : {$qa_hp} <br><br>";

    // mailer($config['cf_admin_email_name'], $qa_email, $qaconfig['qa_admin_email'], $subject, $content, 1); // 원본 코드
    // mailer($config['cf_admin_email_name'], COMMON_SEND_EMAIL, $qaconfig['qa_admin_email'], $subject, $content, 1); // QA설정 메일로 발송처리
    ...
    /* [OSJ : 2024-11-13] 회원정보의 mb_qa_board_cate 와 동일한 카테고리라면 메일 발송 */
    $sql = " select mb_id, mb_email, mb_qa_board_cate from {$g5['member_table']} where mb_qa_board_cate like '%{$qa_category}%' ";
