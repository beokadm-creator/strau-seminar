...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    if ($comment_id)
    {
        ...
        /* [OSJ : 2024-03-11] 댓글 1단계로 제한 */
        if (strlen($reply_array['wr_comment_reply']) == 1)
            alert('답변에 답변하실 수 없습니다.');
    ...
    /* [OSJ : 2024-03-11] 댓글의 대댓글이 달렸다면 작성자에게 메일 발송 */
    if( $w == "c" ){

        $mail_sql = " select * from g5_write_campus where wr_parent = '{$wr_id}' and wr_comment = {$tmp_comment} and wr_is_comment = 1 and wr_comment_reply = '' ";
        $mail_row = sql_fetch($mail_sql);
        if(!empty($mail_row) && $tmp_comment_reply != ""){
            $to_mail = $mail_row["wr_email"];
            $wr_content = nl2br(get_text(stripslashes("원글\n{$mail_row['wr_content']}\n\n\n답변\n$wr_content")));
    
            $subject = '['.$config['cf_title'].'] '.$board['bo_subject'].' Q&A에 답변이 올라왔습니다.';
            ...
            mailer($wr_name, COMMON_SEND_EMAIL, $to_mail, $subject, $content, 1);
        }
    ...
    if ($config['cf_email_use'] && $board['bo_use_email'])
    {
        ...
        /* [OSJ : 2024-03-18] 메일 다중 발송을 위한 코드 변경, 글 등록시 n개의 메일로 발송 처리 : 이메일, 이메일 <-- 메일을 받을 주소는 콤마(,)로 구분 */
        //$mail_addr_list = "oh.sungjae@daum.net, o.sungjae@opencom.com"; // 테스트
		//$mail_addr_list = "fud543@daum.net, ydjang@hongcomm.kr"; // 테스트
        $mail_addr_list = "kevin.jeong@straumann.com, ydjang@hongcomm.kr";
        // mailer($wr_name, $wr_email, $mail_addr_list, $subject, $content, 1);
        mailer($config['cf_admin_email_name'], COMMON_SEND_EMAIL, $mail_addr_list, $subject, $content, 1);
    
        /* [OSJ : 2024-11-13] 회원정보의 mb_board_comment와 bo_table이 같고 mb_board_comment_cate 와 ca_name이 같다면 메일 발송 */
        $sql = " select mb_id, mb_email from g5_member where mb_board_comment = '{$bo_table}' and mb_board_comment_cate like '%{$wr['ca_name']}%' ";
        $result = sql_query($sql);
        $member_mail_addr_list = array();
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            if($row['mb_email'] != "") $member_mail_addr_list[] = $row['mb_email'];
        }
        if(count($member_mail_addr_list) > 0) {
            $member_mail_addr_list = implode(",", $member_mail_addr_list);
            mailer($config['cf_admin_email_name'], COMMON_SEND_EMAIL, $member_mail_addr_list, $subject, $content, 1);
        }
    }
