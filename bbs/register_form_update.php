...
/* [OSJ : 2024-03-08] 아이디와 이메일을 동일하게 사용 */
$mb_email       = $mb_id;
...
if ($w == '' || $w == 'u') {
    ...
    run_event('register_form_update_valid', $w, $mb_id, $mb_email);
...
if ($w == '') {
    // 추천 코드 생성
    $mb_referral_code = generate_referral_code($mb_id);
    ...
    // 추천인 추적 및 기록
    if ($mb_recommend) {
        process_referral_signup($mb_recommend, $mb_id);
    }
    
$referral_code_param = isset($_POST['referral_code']) ? trim($_POST['referral_code']) : '';
if(!$referral_code_param && isset($_COOKIE['referral_code'])) {
    $referral_code_param = trim($_COOKIE['referral_code']);
}
if(!$referral_code_param && function_exists('get_session')) {
    $ref_sess = get_session('referral_code');
    if($ref_sess) $referral_code_param = trim($ref_sess);
}
    if ($referral_code_param) {
        $referrer = get_member_by_referral_code($referral_code_param);
        if ($referrer && $referrer['mb_id'] != $mb_id) {
            process_referral_signup($referrer['mb_id'], $mb_id);
            if (!$mb_recommend) {
                $sql = "UPDATE {$g5['member_table']} SET mb_recommend = '{$referrer['mb_id']}' WHERE mb_id = '{$mb_id}'";
                sql_query($sql);
            }
            $col1 = sql_fetch("SHOW COLUMNS FROM `{$g5['member_table']}` LIKE 'mb_referred_by'");
            if ($col1) {
                sql_query("UPDATE {$g5['member_table']} SET mb_referred_by = '{$referral_code_param}' WHERE mb_id = '{$mb_id}'");
            }
            $col2 = sql_fetch("SHOW COLUMNS FROM `{$g5['member_table']}` LIKE 'mb_referral_count'");
            if ($col2) {
                sql_query("UPDATE {$g5['member_table']} SET mb_referral_count = mb_referral_count + 1 WHERE mb_id = '{$referrer['mb_id']}'");
            }
        }
    }
    ...
    if ($config['cf_email_mb_super_admin']) {
        $subject = run_replace('register_form_update_mail_admin_subject', '['.$config['cf_title'].'] '.$mb_name .' 님께서 회원으로 가입하셨습니다.', $mb_id, $mb_name);
        ...
        run_event('register_form_update_send_admin_mail', $mb_name, $mb_email, $config['cf_admin_email'], $subject, $content);
    }
