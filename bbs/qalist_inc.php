...
if($is_guest)
    alert('회원이시라면 로그인 후 이용해 보십시오.', './login.php?url='.urlencode(G5_BBS_URL.'/mypage.php'));
...
$skin_file = $qa_skin_path.'/list.skin.inc.php';
$is_auth = $is_admin ? true : false;

$category_option = '';

if ($qaconfig['qa_category']) {
    $category_href = G5_BBS_URL.'/mypage.php';
...
if(is_file($skin_file)) {
    ...
    $list_href = G5_BBS_URL.'/mypage.php';
    $write_href = G5_BBS_URL.'/qawrite.php';

    $list_pages = preg_replace('/(\.php)(&amp;|&)/i', '$1?', get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './mypage.php'.$qstr.'&amp;page='));
