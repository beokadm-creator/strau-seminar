...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    $is_guest_allowed = (isset($write['guest_access']) && $write['guest_access'] == 1);

    // 그룹접근 사용
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest && !$is_guest_allowed) {
    ...
    if (!$is_guest_allowed && $member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            alert('글을 읽을 권한이 없습니다.', G5_URL);
        else
            /* [OSJ : 2024-03-12] 로그인 페이지로 이동. */
    ...
    if (!get_session($ss_name))
    {
        //  비회원 조회수 증가 처리 (guest_hit 컬럼이 존재하는 경우만)
        if ($is_guest && array_key_exists('guest_hit', $write)) {
            sql_query(" update {$write_table} set wr_hit = wr_hit + 1, guest_hit = guest_hit + 1 where wr_id = '{$wr_id}' ");
        } else {
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            ...
            goto_url(G5_BBS_URL.'/login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
    }
