...
<?php
  $ret_base = (defined('G5_HTTPS_DOMAIN') && G5_HTTPS_DOMAIN) ? G5_HTTPS_DOMAIN : ( (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? ('https://'.$_SERVER['HTTP_HOST']) : ('http://'.$_SERVER['HTTP_HOST']) );
  $ret_url = rtrim($ret_base, '/') . '/plugin/' . G5_KCPCERT_DIR . $resultPage;
?>
<input type="hidden" name="Ret_URL"      value="<?php echo htmlspecialchars($ret_url, ENT_QUOTES, 'UTF-8'); ?>" />
