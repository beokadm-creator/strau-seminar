...
<div class="subpage qa pb100" id="bo_list">
<div class="inner_container">
    ...
    <!-- [OSJ : 2024-03-08] 목록의 form 안에 있어야 삭제 버튼이 작동함. 임시 인라인 스타일적용하여 처리 -->
    <div style="display: flex; justify-content: end;">
        <?php if ($write_href) { ?><a href="<?php echo $write_href ?>" class="btn_b01 btn" title="문의등록"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sound_only">문의등록</span></a><?php } ?>
        <?php if ($is_checkbox) { ?>
            <button type="submit" name="btn_submit" value="선택삭제" title="선택삭제" onclick="document.pressed=this.value" class="btn btn_b01 btn_admin"><i class="fa fa-trash-o" aria-hidden="true"></i><span class="sound_only">선택삭제</span></button>
        <?php } ?>
    </div>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <?php } ?>
            <!-- <th scope="col">번호</th> -->
						<th scope="col" class="w15">답변상태</th>
            <th scope="col">제목</th>
            <!-- <th scope="col" class="m_dn">글쓴이</th> -->
            <th scope="col" class="m_dn w15">작성일</th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        <tr class="<?php echo $lt_class ?>">
            ...
            <?php } ?>
						<td class="td_stat"><span class=" <?php echo ($list[$i]['qa_status'] ? 'txt_done' : 'txt_rdy'); ?>"><?php echo ($list[$i]['qa_status'] ? '완료' : '대기'); ?></span></td>
            ...
            <td class="td_date"><?php echo date("Y.m.d", strtotime($list[$i]['date'])); ?></td>
        </tr>
        <?php
        }
        ?>
...
$(function(){
    <? if(!empty($page_chk)){ ?>
        $('html, body').animate({scrollTop: $("#bo_list").offset().top},0);
    <? } ?>
});
</script>
<?php } ?>
