...
<button type="button" class="cmt_btn"><span class="total"><b>Q&A</b> <?php echo $view['wr_comment']; ?></span><span class="cmt_more"></span></button>
<!-- 댓글 시작 { -->
<section id="bo_vc">
    <h2>댓글목록</h2>
    <?php
    ...
    for ($i=0; $i<$cmt_amt; $i++) {
        ...
        */
        ...
        /* [OSJ : 2024-03-11] 코멘트 답변 완료처리 */
        $answer_id = "";
        if($cmt_depth > 0){
            $answer_id = 'answer_'.$list[$i]['wr_comment'];
            ?>
            <script>
	...
	<article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        ...
        <div class="cm_wrap">
            ...
            <header style="z-index:<?php echo $cmt_sv; ?>">
	            <h2><?php echo get_text($list[$i]['wr_name']); ?>님의 <?php if ($cmt_depth) { ?><span class="sound_only">댓글의</span><?php } ?> Q&A</h2>
        ...
        if($cmt_depth == 0){ 
        ?>
            <div class="complete_btn" id="<?='answer_'.$list[$i]['wr_comment']?>" style="display:none;"><button>답변완료</button></div>
        <? } ?>
        <?php if($is_comment_reply_edit) { ?>
		<div class="bo_vl_opt">
        	...
        	<ul class="bo_vc_act">
                <?php if ($list[$i]['is_reply'] && ($member['mb_id'] == 'master' || $member['mb_id'] == 'admin')) { ?><li><a href="<?php echo $c_reply_href; ?>" onclick="comment_box('<?php echo $comment_id ?>', 'c'); return false;">답변</a></li><?php } ?>
