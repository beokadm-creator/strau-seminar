...
$sql = "
    SELECT * 
    FROM g5_content_mypage_show c 
        JOIN g5_write_launchingShow s ON s.wr_id = c.content_no 
        LEFT JOIN g5_member m ON m.mb_no = c.user_no 
    {$where} 
    {$sql_order} 
";
...
$headers = array('런칭쇼제목','구분','아이디', '이름', '상태', '휴대폰', '직업', '면허번호', '치과명/학교명', '지역', '제품사용여부', '가입경로', '이메일', '가입일');
...
for ($i=1; $row=sql_fetch_array($result); $i++) {
    ...
    $sql = "select wr_subject from g5_write_launchingShow where wr_id = '{$row["content_no"]}' ";
    $wr = sql_fetch($sql);
    if ($wr) {
        $rows[] = array(
            $wr['wr_subject'],
...
header("Content-Disposition: attachment; filename=\"launchingShow_list-".date("ymdhis", time()).".xls\"");
