...
function get_member($mb_id, $fields='*', $is_cache=false)
{
    ...
    /* [OSJ : 2024-03-08] 아이디 이메일 사용을 위해 검증 제외 처리 */
...
// 한페이지에 보여줄 행, 현재페이지, 총페이지수, URL
function get_paging_list($write_pages, $cur_page, $total_page, $url, $add="")
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#(&amp;)?page=[0-9]*#', '', $url);
	$url .= substr($url, -1) === '?' ? 'page=' : '&amp;page=';
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<li class="prev-first"><a href="'.$url.'1'.$add.'" ><i class="fa fa-angle-double-left" aria-hidden="true"></i></a></li>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<li class=\"prev\"><a href="'.$url.($start_page-1).$add.'"><i class="fa fa-angle-left" aria-hidden="true"></i></a></li>'.PHP_EOL;
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                ...
                $str .= '<li><a class="on" href="#blank">'.$k.'</a></li>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<li class="next"><a href="'.$url.($end_page+1).$add.'"><i class="fa fa-angle-right" aria-hidden="true"></i></a></li>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<li class="next-last"><a href="'.$url.$total_page.$add.'"><i class="fa fa-angle-double-right" aria-hidden="true"></i></a></li>'.PHP_EOL;
    }

    if ($str)
        return "<div class=\"product_paging\"><ul>{$str}</ul></div>";
    else
        return "";
}